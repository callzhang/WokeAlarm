...
@implementation TMTupleNil

+ (TMTupleNil *)tupleNil {
    static dispatch_once_t onceToken;
    static TMTupleNil *tupleNil = nil;
...
+ (instancetype)tupleWithObjectsFromArray:(NSArray *)array convertNullsToNils:(BOOL)convert {
    TMTuple *tuple = [[self alloc] init];

    if (convert) {
        NSMutableArray *newArray = [NSMutableArray arrayWithCapacity:array.count];
        for (id object in array) {
            [newArray addObject:(object == NSNull.null ? TMTupleNil.tupleNil : object)];
        }
...
- (instancetype)tupleByAddingObject:(id)obj {
    NSArray *newArray = [self.backingArray arrayByAddingObject:obj ?: TMTupleNil.tupleNil];
...
@implementation TMTuple (ObjectSubscripting)
...
@implementation TMTupleUnpackingTrampoline
