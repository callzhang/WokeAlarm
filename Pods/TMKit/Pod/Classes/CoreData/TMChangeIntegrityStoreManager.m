//
//  TMChangeIntegrityStoreManager.m
//  Pods
//
...
@interface TMChangeIntegrityStoreManager()
...
@implementation TMChangeIntegrityStoreManager
...
+ (void)storeTICDSSyncID:(NSString *)ticdsSyncID forManagedObjectID:(NSManagedObjectID *)managedObjectID {
    if ([ticdsSyncID length] == 0) {
        DDLogWarn(@"Attempted to store a 0 length ticdsSyncID for managedObject with ID %@", managedObjectID);
        return;
    }
...
+ (NSString *)ticdsSyncIDForManagedObjectID:(NSManagedObjectID *)managedObjectID {
    ...
    if ([syncID length] == 0) {
        DDLogError( @"Retrieved a 0 length ticdsSyncID for managedObject with ID %@", managedObjectID);
    }
...
static TMChangeIntegrityStoreManager *sharedChangeIntegrityStoreManager = nil;
...
+ (TMChangeIntegrityStoreManager *)sharedChangeIntegrityStoreManager
{
