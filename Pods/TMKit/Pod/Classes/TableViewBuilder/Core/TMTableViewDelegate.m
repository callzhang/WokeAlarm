...
- (CGFloat)tableView:(UITableView *)tableView estimatedHeightForRowAtIndexPath:(NSIndexPath *)indexPath NS_AVAILABLE_IOS(7_0) {
    ...
    /**
     *  if section is managed by fetched results controller, rowItem can't be retrived when estimated height 
     * is called. otherwise, row item will be created all at once.
     */
    TMSectionItem *sectionItem = [self.tableViewBuilder sectionItemAtIndex:indexPath.section];
    if (sectionItem.type == TMSectionItemTypeFetchedResultsController) {
        return [sectionItem estimatedHeightForRowAtIndexPath:indexPath];
    }
